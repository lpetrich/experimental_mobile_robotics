#include <game_navigation_layers/seeking_layer.h>
#include <math.h>
#include <angles/angles.h>
#include <pluginlib/class_list_macros.h>
#include "geometry_msgs/PoseWithCovarianceStamped.h"

PLUGINLIB_EXPORT_CLASS(game_navigation_layers::SeekingLayer, costmap_2d::Layer)
...
{
    void SeekingLayer::onInitialize()
    {
        SeekingGameLayer::onInitialize();
        ros::NodeHandle nh("~/" + name_), g_nh;
        server_ = new dynamic_reconfigure::Server<SeekingLayerConfig>(nh);
        f_ = boost::bind(&SeekingLayer::configure, this, _1, _2);
        server_->setCallback(f_);
    }
    
    void SeekingLayer::updateBoundsFromHider(double* min_x, double* min_y, double* max_x, double* max_y)
    {
        try {
            pos_b = hider_position;
            vel_b = hider_velocity;
             
            double mag = sqrt(pow(vel_b.x,2) + pow(vel_b.y, 2));
            double factor = 1.0 + mag * factor_;
            double point = s_get_radius(cutoff_, amplitude_, covar_ * factor );
    ...
    void SeekingLayer::updateCosts(costmap_2d::Costmap2D& master_grid, int min_i, int min_j, int max_i, int max_j){
        boost::recursive_mutex::scoped_lock lock(lock_);

        if(!enabled_) { return; }
        if(!initialized_hider_) { return; }
        if(cutoff_ >= amplitude_) { return; }
        
        costmap_2d::Costmap2D* costmap = layered_costmap_->getCostmap();
        double res = costmap->getResolution();
        pos_c = hider_position;
        vel_c = hider_velocity;
        double angle = atan2(vel_c.y, vel_c.x);
        double mag = sqrt(pow(vel_c.x,2) + pow(vel_c.y, 2));
        double factor = 1.0 + mag * factor_;
        double base = s_get_radius(cutoff_, amplitude_, covar_);
        double point = s_get_radius(cutoff_, amplitude_, covar_ * factor ); 
        ...
        for(int i = start_x; i < end_x; i++) {
            for(int j = start_y; j < end_y; j++) {
                ...
                double a;
                if(fabs(diff) < M_PI/2)
                    a = s_gaussian(x, y, cx, cy, amplitude_, covar_*factor, covar_, angle);
                else
                    a = s_gaussian(x, y, cx, cy, amplitude_, covar_, covar_, 0);
    ...
    void SeekingLayer::configure(SeekingLayerConfig &config, uint32_t level) {
